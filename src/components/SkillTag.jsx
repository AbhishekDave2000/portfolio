...
const SkillTag = ({skill,skillType, style, index}) => {
    ...
    const PRIMARY_SKILLS    = `text-xs md:text-lg text-zinc-700 px-2 py-1 rounded-lg bg-${c(200)}/90 
                                transition-all duration-300 ease-out hover:scale-105 hover:shadow-lg hover:bg-${c(300)} cursor-pointer`;
    const SECONDARY_SKILLS  = `text-xs md:text-lg text-${c(100)} px-2 py-1 rounded-lg border border-${c(700)} 
